describe('Pretty JSON', () => {
  ...
  describe('when prettifying large data file', () =>
    it('does not crash', () =>
      waitsForPromise(() =>
        atom.workspace.open('large.json').then(editor =>
          PrettyJSON.prettify(editor, { sorted: false })))
    ))
  ...
  describe('Sort and prettify JSON file with invalid JSON', () =>
    it('does not change anything', () =>
      waitsForPromise(() =>
        atom.workspace.open('invalid.json')
          .then((editor) => {
            PrettyJSON.prettify(editor, { sorted: true })
            return expect(editor.getText()).toBe(`\
{ "c": "d", "a": "b", }
\
`)
    ...
    it('jsonifies file correctly', () =>
      waitsForPromise(() =>
        atom.workspace.open('object.json')
          .then((editor) => {
            PrettyJSON.jsonify(editor, { sorted: false })
            return expect(editor.getText()).toBe(`\
{
  "c": 3,
  "a": 1
  ...
  describe('JSON file with valid JavaScript Object Literal', () =>
    it('jsonifies and sorts file correctly', () =>
      waitsForPromise(() =>
        atom.workspace.open('object.json')
          .then((editor) => {
            PrettyJSON.jsonify(editor, { sorted: true })
            return expect(editor.getText()).toBe(`\
{
  "a": 1,
  "c": 3
...
`)
  ...
  return xdescribe('when prettifying whole numbers represented as floating point', () =>
